...
import de.comlineag.snc.webcrawler.util.WebCrawlerIO;
...
public class CrawlController extends Configurable {
  ...
  public CrawlController(CrawlConfig config, PageFetcher pageFetcher, RobotstxtServer robotstxtServer)
    ...
    if (!folder.exists()) {
      if (!folder.mkdirs()) {
        throw new Exception("Couldn't create the folder: " + folder.getAbsolutePath());
      }
    }
    ...
    if (!envHome.exists()) {
      if (!envHome.mkdir()) {
        throw new Exception("Couldn't create the folder: " + envHome.getAbsolutePath());
      }
    }
    if (!resumable) {
      WebCrawlerIO.deleteFolderContents(envHome);
    }
