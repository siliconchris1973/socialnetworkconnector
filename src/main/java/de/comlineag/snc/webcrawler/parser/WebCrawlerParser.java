...
import de.comlineag.snc.webcrawler.crawler.Configurable;
import de.comlineag.snc.webcrawler.crawler.CrawlConfig;
import de.comlineag.snc.webcrawler.crawler.Page;
import de.comlineag.snc.webcrawler.url.URLCanonicalizer;
import de.comlineag.snc.webcrawler.url.WebURL;
import de.comlineag.snc.webcrawler.util.WebCrawlerNet;
import de.comlineag.snc.webcrawler.util.WebCrawlerUtil;
 ...
 */
public class WebCrawlerParser extends Configurable {

  protected static final Logger logger = LoggerFactory.getLogger(WebCrawlerParser.class);

  private HtmlParser htmlParser;
  private ParseContext parseContext;

  public WebCrawlerParser(CrawlConfig config) {
    super(config);
  ...
  public boolean parse(Page page, String contextURL) throws NotAllowedContentException {
    ...
    if (WebCrawlerUtil.hasBinaryContent(page.getContentType())) {
      ...
      parseData.setOutgoingUrls(WebCrawlerNet.extractUrls(parseData.getHtml()));
      return parseData.getHtml() != null;
    } else if (WebCrawlerUtil.hasPlainTextContent(page.getContentType())) {
      try {
        ...
        parseData.setOutgoingUrls(WebCrawlerNet.extractUrls(parseData.getTextContent()));
        page.setParseData(parseData);
