...
package de.comlineag.snc.webcrawler.fetcher;
    ...
    try {
      ...
      if (fetchResult.getEntity() != null) {
        ...
        if (size > config.getMaxDownloadSize()) {
          ...
          get.abort();
          logger.warn("Failed: Page Size ("+size+") exceeded max-download-size ("+config.getMaxDownloadSize()+"), at URL: "+ webUrl.getURL());
          return fetchResult;
        }
      ...
      if (toFetchURL.toLowerCase().endsWith("robots.txt")) {
        ...
        logger.error("Fatal transport error: "+e.getMessage() != null ? e.getMessage() : e.getCause()+" while fetching "+toFetchURL+" (link found in doc #" + webUrl.getParentDocid() + ")");
